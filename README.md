...
- 前端页面：
  - `/` 首页，已登录显示模块入口，未登录显示登录按钮
  - `/dashboard` 受保护页面，未登录时自动弹出登录弹窗
  - `/account` 管理员专属页面，非管理员自动跳转到 `/profile`
  - `/profile` 个人中心，所有已登录用户可访问
- 登录态机制：
...
4. 点击页面右上角"登录"按钮，在弹出的登录框中使用初始化的管理员账号登录
5. 登录成功后，页面会显示已登录状态和模块入口
6. 访问 `/account` 页面，确认可以管理用户和角色
7. 点击"退出登录"后再次访问 `/dashboard`，确认会弹出登录框
